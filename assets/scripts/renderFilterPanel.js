...
function renderFilterPanel() {
  ...
  const filterPanelFold = document.createElement("div");
  filterPanelFold.className = "filter-panel__fold";
  ...
  filterPanelFold.appendChild(filterPanelFoldIcon);
  ...
  filterPanel.appendChild(filterPanelFold);
...
function bindFilterFold() {
  // 获取相关元素
  const foldBtn = document.querySelector(".filter-panel__fold");
  const filterPanel = document.querySelector(".filter-panel");
  const filterIcon = document.querySelector(".filter-panel__icon");
  const locationButton = document.querySelector(".filter-location-container");

  // 添加点击事件
  foldBtn.addEventListener("click", () => {
    foldBtn.classList.toggle("filter-panel__fold--active");
    // 切换panel的class
    filterPanel.classList.toggle("filter-panel--hidden");

    filterIcon.classList.toggle("filter-panel__icon--active");
    locationButton.classList.toggle("filter-location--folded");
  });
}

function renderFilterLocationPanel() {
  ...
  // 将创建的结构添加到页面
  document.querySelector(".map-container").appendChild(filterLocationContainer);

  document.querySelectorAll(".filter-location-list-item").forEach((item) => {
